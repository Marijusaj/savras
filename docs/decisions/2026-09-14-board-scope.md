...
**Status:** building; supersedes the M5.1 in-panel view
...
1. **Boards per repository** — `board.rs`: one file per repository that
   ...
   says when there is none, and `c` creates it. *Status:* building
2. **The board as a tab** — a pane kind with a board body; `Row::Board` under
   each repository heading when the board exists, flippable like any tab;
   clean and delete from the panel behind the confirm prompt; the compose line
   kept safe from ctrl-w/s/t. Replaces the M5.1 in-panel view. *Status:* next
