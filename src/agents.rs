...
/// Delete a Codex session, by asking Codex's daemon to: it stops the session
/// if it is running and removes its history. Nothing is killed from here —
/// the process holding a session is the daemon serving every one of them.
pub fn delete_codex(dir: &Path, id: &str) -> Result<String> {
    crate::codex::delete(dir, id)?;
    Ok(String::new())
}
